...
# AssistantCamera / 어시스턴트 카메라

## 1. 기능소개

- **AssistantCamera**는 아바타 커스터마이징 및 각종 화면에서 3D 아바타나 객체를 다양한 시점에서 자동으로 렌더링하는 지능적 카메라 시스템입니다.
- **제작기간**: 2024년 XR 프로젝트 개발 기간 중 지속적으로 개선 및 유지보수

### 개발 배경 및 요구사항
- 아바타 커스터마이징 시스템에서 다양한 부위(얼굴, 의상 등)의 디테일한 확인이 필요한 상황에서 기존 카메라 시스템의 한계로 인해 개발
- 사용자가 특정 부위를 클릭하거나 선택했을 때 해당 부위에 자동으로 카메라가 이동하여 확대/상세 보기 제공
- 3D 객체의 크기와 형태를 분석하여 최적의 카메라 위치를 자동으로 계산하는 지능적 시스템 필요

### 주요 기능
- **자동 위치 계산**: 3D객체의 크기와 형태를 분석하여 최적의 카메라 위치 계산
...
- **플랫폼별 최적화**: iOS와 기타 플랫폼에 대한 성능 최적화
...
- **플랫폼별 조건부 컴파일**을 통한 성능 최적화 (iOS: 안티앨리어싱 1x, 기타: 4x)
