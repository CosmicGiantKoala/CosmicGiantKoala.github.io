...
+++
## 개요
`AchievementState` 열거형은 SlimeRush 게임의 업적 시스템에서 업적 객체([`Achievement`](/docs/projects/SlimeRush/AchievementSystem/Achievement))의 상태를 정의합니다.
이 열거형은 업적 객체의 현재 상태를 추적하고 관리하는 데 사용됩니다. 업적 객체는 비활성 상태에서 시작하여
활성 상태로 전환되고, 최종적으로 완료 상태로 도달합니다.

## 역할
- 업적 객체([`Achievement`](/docs/projects/SlimeRush/AchievementSystem/Achievement))상태 정의
- 상태 전환 로직 지원
- 상태 기반 조건 검사
...
## 기능 설명
### 상태 전환 흐름
- **InActive → Active**: 선행 조건(`Achievement.PreRequireCondition`)을 만족하면 활성화
- **Active → Complete**: 모든 조건(`Achievement.NeedSuccess`)을 달성하면 완료
- **Complete**: 최종 상태, 더 이상 변경되지 않음

### 상태별 특징
- **InActive**: 초기 상태, 선행조건(`Achievement.PreRequireCondition`) 달성 안됨. 조건 달성 불가능
- **Active**: 진행 중인 상태, 조건 달성 가능
- **Complete**: 완료된 상태, 보상 지급 대상

### 상태 관리
- 업적 객체([`Achievement`](/docs/projects/SlimeRush/AchievementSystem/Achievement))가 조건(`Achievement.NeedSuccess`)에 따라 상태 전환 관리
- 상태에 따른 다른 동작 수행
- 상태 기반 UI 표시 변경

## 의존성/상속 관계
- [`Achievement`](/docs/projects/SlimeRush/AchievementSystem/Achievement) 클래스에서 업적 객체 상태에 따라 해당 열거형 제어
