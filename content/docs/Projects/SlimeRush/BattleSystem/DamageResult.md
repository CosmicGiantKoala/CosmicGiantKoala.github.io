+++
title = "DamageResult"
description = "SlimeRush 게임의 전투 시스템에서 데미지 결과를 저장하는 구조체"
icon = "code"
...
+++
## 개요
`DamageResult` 구조체는 SlimeRush 게임의 전투 시스템에서 데미지 결과를 저장하는 구조체입니다. 이 구조체는 공격 받은 대상이 죽었는지 여부, 공격 데미지, 크리티컬 히트 여부, 적용된 데미지, 데미지를 입게 된 타겟 등 다양한 데미지 결과 관련 정보를 담고 있습니다.

## 역할
- 데미지 결과(`float`)를 저장하고 관리
- 공격 받은 대상이 죽었는지 여부(`bool`) 확인
- 크리티컬 히트 여부(`bool`) 확인
...
## 기능 설명
### 데미지 결과 저장
- 공격 받은 대상이 죽었는지 여부(`bool`) 저장
- 플레이어가 몬스터에게 또는 몬스터가 플레이어에게 OnDamaged로 호출한 데미지 수치(`float`) 저장
- 해당 공격이 치명타로 적용되었는지 여부(`bool`) 저장
- 데미지 감소 방어 등이 적용된 이후의 최종 데미지 값(`float`) 저장
- 데미지를 입게 된 게임오브젝트(`GameObejct`) 저장

## 사용 예시
### [`DamageInfo`](/docs/projects/SlimeRush/BattleSystem/DamageInfo)를 기반으로 데미지를 계산하고, `DamageResult`를 반환
```csharp
/// <summary>
