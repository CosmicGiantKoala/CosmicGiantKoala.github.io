+++
title = "ITarget"
description = "전투 시스템에서 타겟을 정의하는 인터페이스"
icon = "code"
...
# ITarget
## 개요
`ITarget` 인터페이스는 SlimeRush 게임의 전투 시스템에서 타겟을 정의합니다. 이 인터페이스는 타겟의 위치 반환, 타겟의 Transform 컴포넌트 반환, IDamageAble 인터페이스를 구현하는 오브젝트 인스턴스 반환 등의 기능을 제공합니다.

## 역할
- 타겟의 위치 반환
- 타겟의 Transform 컴포넌트 반환
- IDamageAble 인터페이스를 구현하는 오브젝트 인스턴스 반환
...
## 기능 설명
### 타겟의 위치 반환
- 타겟의 위치를 Vector3 형태로 반환

### 타겟의 Transform 컴포넌트 반환
- 타겟의 Transform 컴포넌트를 반환

### IDamageAble 인터페이스를 구현하는 오브젝트 인스턴스 반환
- IDamageAble 인터페이스를 구현하는 오브젝트를 반환
...
## 사용 예시
#### `TargetScanner`에서 범위 내의 타겟을 서치할 때 사용
```csharp
...
#### `MagicEffect`에서 마법이 발동될 때 시전자의 `Transform`을 기반으로 동작을 설정하기 위해 사용
```csharp
...
## 관련 클래스
- `IDamageAble`: 대미지 가능 오브젝트를 나타내는 인터페이스
